...
export default async function BlogPostPage({
  ...
  const seoLabels = seoArticleLabels[blogLocale];
  const articleSources = getArticleSources(post, contentHtml, locale);
  const coconLinks = getArticleCocoonLinks(post, locale);
  const articleKeywords = getArticleKeywords(post);
  ...
  const blogPostingSchema = {
    ...
    isBasedOn: articleSources
      .filter((source) => source.kind === 'federal' || source.kind === 'official')
      .slice(0, 5)
      .map((source) => ({
        '@type': 'CreativeWork',
        name: source.title,
  ...
  return (
    <section className="min-h-screen bg-gray-light">
      ...
      <article className="mx-auto max-w-4xl px-4 pb-20 pt-4 sm:px-6 lg:px-8">
        ...
        <BlogSeoSections labels={seoLabels} coconLinks={coconLinks} sources={articleSources} />
