                ...
                <div className="mt-6 space-y-3 text-left">
                  {(['noWaiting', 'autoConfirm', 'syncCalendar'] as const).map((item) => (
                    <div key={item} className="flex items-center gap-3">
                      <CheckIcon />
                      <span className="text-sm text-gray-700">{t(`hero.${item}`)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {steps.map((step, i) => (
              <div key={step} className="relative">
                {i < steps.length - 1 && (
                  <div className="absolute left-5 top-10 hidden h-0.5 w-full bg-gray-200 lg:block" />
                )}
                <div className="relative rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-shadow duration-200 hover:shadow-md">
