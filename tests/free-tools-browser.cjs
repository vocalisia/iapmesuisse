...
await page.setViewportSize({width:390,height:844});await page.getByRole('button',{name:/Kit de charte IA/}).click();await page.locator('a#gain-temps').click();await page.getByRole('button',{name:'Calculer mon résultat'}).click();await page.locator('#active-tool').scrollIntoViewIfNeeded();await page.screenshot({path:dir+'/mobile-tools.png'});check('mobile no overflow',await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
